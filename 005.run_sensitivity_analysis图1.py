...
# --- 敏感性分析配置 ---
K_CANDIDATES = np.array([3], dtype=np.int64)
...
def save_results(hypo_space, feat_dim,
    ...
    np.savez_compressed("sep_analysis_results_P1.npz", **save_data)
    print("\n所有结果已保存至 sep_analysis_results_P1.npz")
...
if __name__ == "__main__":
    ...
    # 3. 单独保存信念演化数据（专供图1使用）
    np.savez_compressed(
        "belief_evolution_raw.npz",
        beliefs=cp.asnumpy(sep_beliefs),
        opt_ells=cp.asnumpy(sep_opt_ells),
        final_ell=sep_final_ell,
        hypothesis_space=cp.asnumpy(hypo_space),
        grid_size=GRID_SIZE,
        channels_per_bin=CHANNELS_PER_BIN
    )
    print("\n 信念演化数据已保存至 belief_evolution_raw.npz")
